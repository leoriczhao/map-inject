// UnitState.cpp — EX* unit state natives
// Ported from YDWE yd_jass_api/UnitState.cpp
// Provides EXGetUnitString, EXSetUnitString, EXGetUnitReal, EXSetUnitReal,
// EXGetUnitInteger, EXSetUnitInteger, EXGetUnitArrayString, EXSetUnitArrayString,
// EXPauseUnit, EXSetUnitCollisionType, EXSetUnitMoveType, EXSetUnitFacing.
// Also hooks GetUnitState/SetUnitState for extended unit properties.
...
namespace warcraft3::japi {
	...
	enum UNIT_STATE
	{
		...
		UNIT_STATE_RATE_OF_FIRE                 = 0x51,
		UNIT_STATE_ACQUISITION_RANGE            = 0x52,
	...
	uint32_t __cdecl FakeGetUnitState(uint32_t unit_handle, uint32_t state_type)
	{
		switch (state_type)
		{
	...
	void __cdecl FakeSetUnitState(uint32_t unit_handle, uint32_t state_type, uint32_t* value_ptr)
	{
		switch (state_type)
		{
		case UNIT_STATE_ATTACK1_ATTACK_TYPE:
	...
	void __cdecl EXPauseUnit(jass::jhandle_t unit_handle, jass::jboolean_t flag)
	{
	...
	uint32_t __cdecl EXGetObject(uint32_t unit_handle)
	{
	...
	jass::jstring_t __cdecl EXGetUnitString(jass::jinteger_t unitcode, jass::jinteger_t type)
	{
	...
	jass::jboolean_t __cdecl EXSetUnitString(jass::jinteger_t unitcode, jass::jinteger_t type, jass::jstring_t value)
	{
		uintptr_t ptr = GetUnitData(unitcode);
	...
	jass::jreal_t __cdecl EXGetUnitReal(jass::jinteger_t unitcode, jass::jinteger_t type)
	{
		uintptr_t ptr = GetUnitData(unitcode);
		return jass::to_real(*(float*)(ptr + type * 4));
	}

	jass::jboolean_t __cdecl EXSetUnitReal(jass::jinteger_t unitcode, jass::jinteger_t type, jass::jreal_t* value)
	{
		uintptr_t ptr = GetUnitData(unitcode);
	...
	jass::jinteger_t __cdecl EXGetUnitInteger(jass::jinteger_t unitcode, jass::jinteger_t type)
	{
		uintptr_t ptr = GetUnitData(unitcode);
		return *(jass::jinteger_t*)(ptr + type * 4);
	}

	jass::jboolean_t __cdecl EXSetUnitInteger(jass::jinteger_t unitcode, jass::jinteger_t type, jass::jinteger_t value)
	{
		uintptr_t ptr = GetUnitData(unitcode);
	...
	jass::jstring_t __cdecl EXGetUnitArrayString(jass::jinteger_t unitcode, jass::jinteger_t type, jass::jinteger_t index)
	{
		uintptr_t ptr = GetUnitData(unitcode);
	...
	jass::jboolean_t __cdecl EXSetUnitArrayString(jass::jinteger_t unitcode, jass::jinteger_t type, jass::jinteger_t index, jass::jstring_t value)
	{
		uintptr_t ptr = GetUnitData(unitcode);
		jass::jinteger_t size = *(jass::jinteger_t*)(ptr + type * 4);
		if (size <= 0)
	...
	jass::jnothing_t __cdecl EXSetUnitFacing(jass::jhandle_t unit, jass::jreal_t* pangle)
	{
	...
	jass::jnothing_t __cdecl EXSetUnitCollisionType(jass::jboolean_t enable, jass::jhandle_t unit, jass::jinteger_t type)
	{
	...
	jass::jnothing_t __cdecl EXSetUnitMoveType(jass::jhandle_t unit, jass::jinteger_t type)
	{
		uint32_t uobj = handle_to_object(unit);
		if (!uobj)
		{
