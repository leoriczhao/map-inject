// CDirect3DDevice9.h — Proxy wrapper for IDirect3DDevice9
//
// Hooks EndScene() for custom rendering and Reset() for device-lost handling.
// All other methods are forwarded to the real IDirect3DDevice9.
...
class CDirect3DDevice9 : public IDirect3DDevice9
{
public:
    CDirect3DDevice9(IDirect3DDevice9* pOriginal);
    virtual ~CDirect3DDevice9();
    ...
    // IDirect3DDevice9 — forwarded (in vtable order)
    ...
    // Hooked: Reset() — release/recreate D3D resources
    ...
    // Hooked: EndScene() — render custom objects
...
private:
    IDirect3DDevice9* m_pDevice;  // The real IDirect3DDevice9
